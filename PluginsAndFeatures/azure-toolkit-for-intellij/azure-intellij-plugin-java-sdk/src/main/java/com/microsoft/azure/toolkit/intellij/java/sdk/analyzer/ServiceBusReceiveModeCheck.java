...
/**
 * This class extends the LocalInspectionTool and is used to inspect the usage of Azure SDK ServiceBus &
 * ServiceBusProcessor clients in the code. It checks if the receive mode is set to PEEK_LOCK and the prefetch count is
 * set to a value greater than 1. If the receive mode is set to PEEK_LOCK and the prefetch count is set to a value
 * greater than 1, a problem is registered with the ProblemsHolder.
 */
public class ServiceBusReceiveModeCheck extends LocalInspectionTool {
     ...
     * usage of Azure SDK ServiceBusReceiver & ServiceBusProcessor clients and whether the receive mode is set to
     * PEEK_LOCK and the prefetch count is set to a value greater than 1. If the receive mode is set to PEEK_LOCK and
     * the prefetch count is set to a value greater than 1, a problem is registered with the ProblemsHolder.
     */
    static class ServiceBusReceiveModeVisitor extends JavaElementVisitor {
        ...
        /**
         * This method is used to visit the declaration statement in the code. It checks for the declaration of the
         * Azure SDK ServiceBusReceiver & ServiceBusProcessor clients and whether the receive mode is set to PEEK_LOCK
         * and the prefetch count is set to a value greater than 1. If the receive mode is set to PEEK_LOCK and the
         * prefetch count is set to a value greater than 1, a problem is registered with the ProblemsHolder.
         *
        ...
        /**
         * This method is used to process the variable declaration. It checks if the client type is an Azure ServiceBus
         * client and retrieves the client name (left side of the declaration).
         *
        ...
        private void processVariableDeclaration(PsiVariable variable) {
            ...
            isRelevantClientType(variable.getType());

            PsiExpression initializer = variable.getInitializer();
            if (initializer instanceof PsiMethodCallExpression methodCall) {
                analyzeMethodCallChain(methodCall);
            }
        }

        private boolean isRelevantClientType(PsiType type) {
            String typeName = type.getCanonicalText();
            return typeName.startsWith(RuleConfig.AZURE_PACKAGE_NAME) || RULE_CONFIG.getScopeToCheck().contains(typeName);
        }

        /**
         * This method is used to determine the receive mode of the Azure ServiceBus client. It checks if the receive
         * mode is set to PEEK_LOCK and the prefetch count is set to a value greater than 1. If the receive mode is set
         * to PEEK_LOCK and the prefetch count is set to a value greater than 1, a problem is registered with the
         * ProblemsHolder.
         *
         * @param methodCall The method call expression to check
         */
        private void analyzeMethodCallChain(PsiMethodCallExpression methodCall) {
            ...
            while (qualifier instanceof PsiMethodCallExpression) {
                ...
                if ("receiveMode".equals(methodName)) {
                    ...
                    prefetchCountValue = extractPrefetchCount((PsiMethodCallExpression) qualifier);
                    prefetchCountMethod =
                        ((PsiMethodCallExpression) qualifier).getMethodExpression().getReferenceNameElement();
                }
         ...
         */
        private boolean receiveModePeekLockCheck(PsiMethodCallExpression methodCall) {
            for (PsiExpression arg : methodCall.getArgumentList().getExpressions()) {
                if ("PEEK_LOCK".equals(arg.getText())) {
                    return true;
                }
            }
            return false;
         ...
         */
        private OptionalInt extractPrefetchCount(PsiMethodCallExpression methodCall) {
            PsiExpression[] args = methodCall.getArgumentList().getExpressions();
            if (args.length > 0) {
                try {
                    return OptionalInt.of(Integer.parseInt(args[0].getText()));
                } catch (NumberFormatException e) {
                    LOGGER.log(Level.SEVERE, "Invalid prefetch count: " + args[0].getText(), e);
                }
            }
